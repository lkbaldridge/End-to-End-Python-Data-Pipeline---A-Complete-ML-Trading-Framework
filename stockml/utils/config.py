...
model_names = ['hoeffding_adaptive']
...
def get_drift_params(trial, drift_detector):
    ...
    raise ValueError(f"Unsupported drift detector: {drift_detector}")


def remove_excluded_params(default_params, exclude_params):
    """
    Remove excluded parameters from the default parameter configuration.
    """
...
def apply_custom_param_ranges(param_ranges, default_params):
    """
    Apply custom parameter ranges to the default parameter configuration.
    """
    updated_params = default_params.copy()
    ...
    if param_ranges:
        for param_name, values in param_ranges.items():
            if param_name in updated_params:
                updated_params[param_name] = values
...
def suggest_hyperparameter(trial, param_name, param_type, param_values):
    """
    Suggest an Optuna hyperparameter based on its configured type.
    """
    if param_type == 'int':
...
def get_model_params(
    ...
    if model_name == 'hoeffding_adaptive':
        default_params = param_spaces[model_name].copy()

        default_params = remove_excluded_params(
            default_params,
            exclude_params
        )

        default_params = apply_custom_param_ranges(
            param_ranges,
            default_params
        )
        ...
        for param_name, (param_type, param_values) in default_params.items():
            trial_params_dict[param_name] = suggest_hyperparameter(
                trial,
                param_name,
                param_type,
                param_values
        ...
        trial_params_dict['drift_detector'] = get_drift_params(
            trial,
            'ADWIN'
        )

        return river.tree.HoeffdingAdaptiveTreeClassifier(
            **trial_params_dict,
            seed=42
        )
